package fr.javafx.temp.graphics.fx.api.database;
...
public class DatabaseFX<T> extends TableView<T> {
    Class<T>          clazz  = null;
...
//	    TableColumn<T, String> column =  (TableColumn<T, String>) table.getColumns().get(0);
	    TableColumn<T, String> column =  (TableColumn<T, String>) getColumns().get(0);
...
//			table.setItems( data = FXCollections.observableArrayList(tempList) );
			setItems( data = FXCollections.observableArrayList(tempList) );
			buffer.clear();
		}
