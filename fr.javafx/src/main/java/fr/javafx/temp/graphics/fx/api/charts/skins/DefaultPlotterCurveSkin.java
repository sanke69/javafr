...
public class DefaultPlotterCurveSkin implements Skin<ChartControl> {
	...
	public DefaultPlotterCurveSkin(ChartControl _control) {
		super();
		control = _control;
	...
	class Plot extends Pane {
		...
		public Plot(Double[] _x, Double[] _y, Axes _axes) {
			...
			path.getElements().add(new MoveTo(mapX(_x[0], _axes), mapY(_y[0], _axes)));
			for(int i = 1; i < _x.length; ++i)
				path.getElements().add(new LineTo(mapX(_x[i], _axes), mapY(_y[i], _axes)));
