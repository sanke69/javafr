...
public final class InstantAxis extends Axis<Instant> {
	...
	public static Axis<Number> forLong() {
		...
		xAxis.setLabel( "date" );
		xAxis.setTickLabelFormatter(XY.defaultDateFormatter( "dd/MM/yyyy" ) );
		return xAxis;
	}
	
	enum Interval {
		DECADE		(Period.ofYears(10),     Time.Unit.DECADE),
        YEAR		(Period.ofYears(1),      Time.Unit.YEAR),
        MONTH_6		(Period.ofMonths(6),     Time.Unit.MONTH),
        MONTH_3		(Period.ofMonths(3),     Time.Unit.MONTH),
        MONTH_1		(Period.ofMonths(1),     Time.Unit.MONTH),
        WEEK		(Period.ofDays(7),       Time.Unit.WEEK),
        DAY			(Period.ofDays(1),       Time.Unit.DAY),
        HOUR_12		(Duration.ofHours(12),   Time.Unit.HOUR),
        HOUR_6		(Duration.ofHours(6),    Time.Unit.HOUR),
        HOUR_3		(Duration.ofHours(3),    Time.Unit.HOUR),
        HOUR_1		(Duration.ofHours(1),    Time.Unit.HOUR),
        MINUTE_30	(Duration.ofHours(1),    Time.Unit.MINUTE),
        MINUTE_15	(Duration.ofHours(1),    Time.Unit.MINUTE),
        MINUTE_5	(Duration.ofHours(1),    Time.Unit.MINUTE),
        MINUTE_1	(Duration.ofHours(1),    Time.Unit.MINUTE),
        SECOND_30	(Duration.ofSeconds(30), Time.Unit.SECOND),
        SECOND_15	(Duration.ofSeconds(15), Time.Unit.SECOND),
        SECOND_5	(Duration.ofSeconds(5),  Time.Unit.SECOND),
        SECOND_1	(Duration.ofSeconds(1),  Time.Unit.SECOND),
        MILLISECOND	(Duration.ofMillis(1),   Time.Unit.MINUTE);

		Duration duration;
		Period   period;
		Time.Unit unit;

		private Interval(Duration _duration, Time.Unit _tu) {
			duration = _duration;
			period   = null;
			unit     = _tu;
		}
		private Interval(Period _period, Time.Unit _tu) {
			duration = null;
			period   = _period;
			unit     = _tu;
		}
    ...
    public final Time.Unit					getCurrentTimeUnit() {
    ...
    protected String 						getTickMarkLabel(Instant date) {
        ...
        if (actualInterval.unit == Time.Unit.YEAR && ldt.getMonthValue() == 1 && ldt.getDayOfMonth() == 1) {
            dateFormat = DateTimeFormatter.ofPattern("uuuu");
        } else if (actualInterval.unit == Time.Unit.MONTH && ldt.getDayOfMonth() == 1) {
