...
public abstract class XYValueAxis extends ValueAxis<Number> implements XY.Axis<Number> {
    ...
    protected void 											setRange(Object rangeObj, boolean animate) {
    	if(rangeObj instanceof XY.Axis.Range) {
            ...
            currentUpperBound.set(range.upperBound());
            ...
            super.setScale(range.scale());
    	} else
    		throw new IllegalArgumentException();
    }
    @Override
    protected XY.Axis.Range 								getRange() {
        return XY.Axis.Range.of(getLowerBound(), getUpperBound(), getScale(), ""/*currentTickFormat.get()*/);
    }
...
*/
    	...
    	if(formatter instanceof XY.Axis.Ticks.Formatter<?>) {
            labelText = ((XY.Axis.Ticks.Formatter<?>) formatter).numberToString(value);
        } else
