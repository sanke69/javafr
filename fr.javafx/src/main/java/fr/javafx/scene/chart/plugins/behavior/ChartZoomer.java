...
public class ChartZoomer<TX, TY> extends AbstractBehaviorPlugin<TX, TY> {

	public ChartZoomer() {
		this(null);
	}
	public ChartZoomer(Predicate<InputEvent> _filter) {
		super(_filter);
		registerScrollEventHandler ( ScrollEvent.ANY, this::onMouseScroll );
	}

	public void 			onMouseScroll( ScrollEvent event ) {
		if(getDefaultInputFilter() != null && !getDefaultInputFilter().test(event))
			return ;
		...
		if ( eventType == ScrollEvent.SCROLL && !event.isInertia() && event.getDeltaY() != 0 && event.getTouchCount() == 0 ) {
			...
			XY.Constraint zoomMode = getInteractionStrategy().getConstraint( XYChartUtils.getContext(getChartPane(), eventX, eventY) );
    ...
    public static final <TX, TY> void 		performZoom(XY.Chart<TX, TY> chart, Rectangle2D zoomWindow, XY.Constraint _mode) {
    ...
    public final void		performZoomX(XYChart<TX, TY> _chart, double _x_anchor, double _y_anchor, double _factor) {
    ...
    public final void		performZoomY(XYChart<TX, TY> _chart, double _x_anchor, double _y_anchor, double _factor) {
		Point2D dataCoords   = XYChartUtils.getDataCoordinates( getChartPane(), _chart, _x_anchor, _y_anchor );
