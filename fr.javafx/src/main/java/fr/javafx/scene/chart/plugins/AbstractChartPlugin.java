...
public abstract class AbstractChartPlugin<X, Y> implements XY.ChartPlugin<X, Y> {
    ...
    private final Point2D 							toDisplayPoint_d	(Axis<Y> yAxis, Data<X, Y> dataPoint) {
    	double px = getChartPane().getXYChart().getXAxis().getDisplayPosition( dataPoint.getXValue() );
    	double py = yAxis.getDisplayPosition( dataPoint.getYValue() );

        return new Point2D(px, py);
    }
    private final Data<X, Y> 						toDataPoint_d		(Axis<Y> yAxis, Point2D displayPoint) {
        return new Data<>(
        			getChartPane().getXYChart().getXAxis().getValueForDisplay( displayPoint.getX()), 
