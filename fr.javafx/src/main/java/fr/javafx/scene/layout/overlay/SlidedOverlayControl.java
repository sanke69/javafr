 ...
 */
package fr.javafx.scene.layout.overlay;
...
public class SlidedOverlayControl extends Control implements EventHandler<ActionEvent> {
    ...
    ChangeListener<Number> 	slideUpdater = (_obs, _old, _new) -> {
    	...
    	switch(slideDir) {
    	case LEFT     : slideBeg =  0;
    	case UP       : slideBeg =  0;
		case RIGHT    : slideBeg =  substitute.getWidth()  - slideBounds.getWidth();
		case DOWN     : slideBeg =  substitute.getHeight() - slideBounds.getHeight();      	
    	};
    	switch(slideDir) {
		case LEFT     : slideEnd = - slideBounds.getWidth();
		case RIGHT    : slideEnd =   substitute.getWidth();
		case UP       : slideEnd = - slideBounds.getHeight();
		case DOWN     : slideEnd =   substitute.getHeight();    	
    	};
		...
		switch(slideDir) {
		case LEFT   :
		case  RIGHT : 		overlay . setLayoutX    (slideBeg);
				            ...
				            break;
		case UP      :
		case DOWN    :  	overlay . setLayoutX    (slideBounds.getX());
    ...
    public SlidedOverlayControl(Node _overlayed, TranslateDirection _dir, double _dim) {
		...
		case LEFT  : 
		case RIGHT : initSlidePane(0, 0, _dim, -1); break;
		case UP    : 
		case DOWN  : initSlidePane(0, 0, -1, _dim); break;
    	};
    ...
    public SlidedOverlayControl(Node _overlayed, TranslateDirection _dir, double _offset, double _width, double _height) {
		...
		switch(slideDir = _dir) {
		case LEFT  : 
		case RIGHT : initSlidePane(0, _offset, _width, _height); break;
		case UP    : 
		case DOWN  : initSlidePane(_offset, 0, _width, _height); break;
    	};
    	slideUpdater.changed(null, 0, 0);
    ...
    public void 		setSlideOffset(double _offset) {
    	switch(slideDir) {
		case LEFT  :
		case RIGHT : slideBounds = new Rectangle(_offset, 0, slideBounds.getWidth(), slideBounds.getHeight());
		case UP    :
		case DOWN  : slideBounds = new Rectangle(0, _offset, slideBounds.getWidth(), slideBounds.getHeight());
    	};
    }
