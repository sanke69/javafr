...
//        });
    ...
    private void selectIntersectingNodes(Parent root, boolean deselect) {
        ...
        List<Node> selectableNodes = root.getChildrenUnmodifiable().filtered(n -> n instanceof Selectable);
        ...
        for (Node n : selectableNodes) {
            ...
            Selectable sn = (Selectable) n;

            if ((deselect || potentiallySelected(sn)) || (selectN && rectBigEnough)) {
            	Clipboard.SelectableItem.select(sn, (selectN && rectBigEnough));
            }
        }
    ...
    public void performDragBegin(
        ...
        selectedNodes.addAll(Clipboard.SelectableItem.getSelectedItems());
        ...
        FxNodeUtils.addToParent(root, rectangle);
    ...
    public void performDragEnd(
        ...
        if (rectangle.getParent() != null) {
            ...
            FxNodeUtils.removeFromParent(rectangle);
    ...
    private boolean potentiallySelected(Selectable sn) {
        return !selectedNodes.contains(sn)
                && Clipboard.SelectableItem.getSelectedItems().contains(sn);
    }
}
