package fr.javafx.behavior.extended.listeners;
...
public final class MultiplePropertyChangeListenerHandler {
	...
	public MultiplePropertyChangeListenerHandler(Callback<String, Void> _propertyChangedHandler) {
		propertyChangedHandler = _propertyChangedHandler;
	}

	/**
	 * This is part of the workaround introduced during delomboking. We probably
	 * will want to adjust the way listeners are added rather than continuing to use
	 * this map (although it doesn't really do much harm).
	 */
	...
	private final ChangeListener<Object> propertyChangedListener = new ChangeListener<Object>() {
		@Override
		public void changed(ObservableValue<?> property, @SuppressWarnings("unused") Object oldValue, @SuppressWarnings("unused") Object newValue) {
			propertyChangedHandler.call(propertyReferenceMap.get(property));
		}
	};

	private final WeakChangeListener<Object> weakPropertyChangedListener = new WeakChangeListener<Object>(propertyChangedListener);
