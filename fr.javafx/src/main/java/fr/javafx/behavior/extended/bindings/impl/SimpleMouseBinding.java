...
public final class SimpleMouseBinding<B extends Behavior<?>> extends AbstractBinding<MouseEvent, B> {

  private final Collection<MouseInput> inputs;

  public SimpleMouseBinding(Collection<MouseInput> inputs, EventType<MouseEvent> eventType, Consumer<B> action) {
   ...
   */
  public static <B extends Behavior<?>> MouseBindingBuilder<B> builder() {
    return new MouseBindingBuilder<>();
  }

  public static final class MouseBindingBuilder<B extends Behavior<?>>
      extends Builder<MouseEvent, B, SimpleMouseBinding<B>> {
    ...
    @Override
    public SimpleMouseBinding<B> build() {
      return new SimpleMouseBinding<>(customInputs == null ? DEFAULT_INPUTS : customInputs, eventType, action);
    }
  }
  ...
  public static final class MouseInput {
    ...
    private boolean modifiersMatch(MouseEvent event) {
      return (event.isControlDown()  == (keys.getControl()  != KeyCombination.ModifierValue.UP))
          && (event.isShiftDown()    == (keys.getShift()    != KeyCombination.ModifierValue.UP))
          && (event.isAltDown()      == (keys.getAlt()      != KeyCombination.ModifierValue.UP))
