...
/*/
		final LineChart<Number, Number> sc0 = new LineChart<Number, Number>(xAxis, yAxis) {
			// Override to remove symbols on each data point
			@Override protected void dataItemAdded(Series<Number, Number> series, int itemIndex, Data<Number, Number> item) { }
		};
/**/
...
//			@Override protected void dataItemAdded(Series<Number, Number> series, int itemIndex, Data<Number, Number> item) { }
    ...
    private void addDataToSeries() {
        for (int i = 0; i < 30; i++) { //-- add 20 numbers to the plot+
            if (dataQ.isEmpty()) break;
            series.getData().add(new AreaChart.Data(Instant.now().toEpochMilli(), dataQ.remove()));
        }
    	...
    	Instant t0 = Instant.now().minus(duration);
    	Instant t1 = Instant.now();
        // update 
        xAxis.setLowerBound(t0.toEpochMilli());
        xAxis.setUpperBound(t1.toEpochMilli());
    }
}