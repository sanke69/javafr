...
public class RasterDrawerBufferedImageIntRGB extends RasterDrawerBufferedImage {
	private final int[] 		buffer;

	public RasterDrawerBufferedImageIntRGB(XRasterBufferedImage _raster) {
		super(_raster);
	...
	public byte getRed(int _i, int _j)   			{ return (byte) (buffer[(int) (_j * width + _i)]); }
	public byte getGreen(int _i, int _j) 			{ return (byte) (buffer[(int) (_j * width + _i)] >>  8); }
	public byte getBlue(int _i, int _j)  			{ return (byte) (buffer[(int) (_j * width + _i)] >> 16); }

	public void drawRaster(Canvas _canvas, Viewport2D<?, ?> _viewport) {
		int   mw     = (int)   width;
		int   mh     = (int)   height;
		int   md     = (int)   depth;
			...
			if(rj >= 0 && rj < mh && !(rj == 0 && _yOnRaster < 0))
				for(int x = ww; x-- != 0;) {
					...
					if((ri >= 0 && ri < mw) && (rj >= 0 && rj < mh))
						switch(cd) {
						case 1 : 	dst[pos]     = (byte) (buffer[(int) (depth * (rj * width + ri))]);
									break;
						case 3 : 	dst[pos]     = (byte) (buffer[(int) (rj * width + ri)]);
									dst[pos + 1] = (byte) (buffer[(int) (rj * width + ri)] >>  8);
									dst[pos + 2] = (byte) (buffer[(int) (rj * width + ri)] >> 16);
									break;
						case 4 : 	dst[pos]     = (byte) (buffer[(int) (rj * width + ri)]);
									dst[pos + 1] = (byte) (buffer[(int) (rj * width + ri)] >>  8);
									dst[pos + 2] = (byte) (buffer[(int) (rj * width + ri)] >> 16);
									break;
						}
	...
	public void drawValues(Canvas _canvas, Viewport2D<?, Point2D> _viewport) {
		GraphicsContext     	gc   = _canvas.getGraphicsContext2D();
		Viewport2D<?, Point2D>	vp   = _viewport;
		...
		for(int i = I0; i <= I1; i++)
			for(int j = J0; j <= J1; j++) {
				...
				float gap = 1.0f / (rd + 1);
				for(int k = 0; k < rd; k++) {
					byte value = (byte) (buffer[(int) (j * width + i)] >> (k * 8));

					switch (rd) {
					case 1: 	gc.setFill((value > 127) ? Color.BLACK : Color.WHITE); break;
					case 3: 	switch (k) {
								case 0  : gc.setFill((value > 127) ? new Color(0, 0, 0.5, 1) : Color.BLUE); break;
								case 1  : gc.setFill((value > 127) ? new Color(0, 0.5, 0, 1) : Color.LIME); break;
								case 2  : gc.setFill((value > 127) ? new Color(0.5, 0, 0, 1) : Color.RED); break;
								default : break;
								}
					default: 	break;
					}
