package fr.drawer.fx.raster.awt.bytes;
...
public class RasterDrawerBufferedImageByteABGR extends RasterDrawerBufferedImage {
	
	private final BufferedImage bimage;
	private final byte[] 		buffer;

	public RasterDrawerBufferedImageByteABGR(XRasterBufferedImage _raster) {
		super(_raster);
		...
		if(type != BufferedImage.TYPE_4BYTE_ABGR || !(dbuffer instanceof DataBufferByte))
			throw new RuntimeException();

		buffer = ((DataBufferByte) dbuffer).getData();
	}

	public byte   	 	getValue(int _i, int _j, int _k)	{ return buffer[(int) (depth * (_j * width + _i) + _k)]; }
	
	public byte   	 	getLuminance(int _i, int _j)   		{ return buffer[(int) (depth * (_j * width + _i))]; }
	
	public byte   	 	getRed(int _i, int _j)   			{ return buffer[(int) (depth * (_j * width + _i) + 3)]; }
	public byte    	 	getGreen(int _i, int _j) 			{ return buffer[(int) (depth * (_j * width + _i) + 2)]; }
	public byte  	 	getBlue(int _i, int _j)  			{ return buffer[(int) (depth * (_j * width + _i) + 1)]; }
/*
