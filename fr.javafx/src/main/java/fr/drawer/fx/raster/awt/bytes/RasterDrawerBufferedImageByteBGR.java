...
public class RasterDrawerBufferedImageByteBGR extends RasterDrawerBufferedImage {
	
	private final BufferedImage bimage;
	private final byte[] 		buffer;

	public RasterDrawerBufferedImageByteBGR(XRasterBufferedImage _raster) {
		super(_raster);
		...
		if(type != BufferedImage.TYPE_3BYTE_BGR || !(dbuffer instanceof DataBufferByte))
	...
	public byte getRed(int _i, int _j)   			{ return buffer[(int) (depth * (_j * width + _i) + 2)]; }
	public byte getGreen(int _i, int _j) 			{ return buffer[(int) (depth * (_j * width + _i) + 1)]; }
