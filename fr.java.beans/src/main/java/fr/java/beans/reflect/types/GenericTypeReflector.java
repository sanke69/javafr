...
public class GenericTypeReflector {
	private static final Type UNBOUND_WILDCARD = TypeFactory.createWildcardType(new Type[]{Object.class}, new Type[]{});
	...
	public static Class<?> erase(Type type) {
		if (type instanceof Class) {
			...
			return TypeFactory.createArrayClass(erase(aType.getGenericComponentType()));
		} else {
	...
	private static Type mapTypeParameters(Type toMapType, Type typeAndParams) {
		if (isMissingTypeParameters(typeAndParams)) {
			...
			TypeVariableMap varMap = new TypeVariableMap();
	...
	public static Type addWildcardParameters(Class<?> clazz) {
		if (clazz.isArray()) {
			return TypeFactory.createGenericArrayType(addWildcardParameters(clazz.getComponentType()));
			...
			return TypeFactory.createParameterizedType(clazz, arguments, owner);
		} else {
			return clazz;
		}
	}
	...
	private static Type[] getArrayExactDirectSuperTypes(Type arrayType) {
		...
		if (typeComponent instanceof Class && ((Class<?>)typeComponent).isPrimitive()) {
			...
			for (resultIndex = 0; resultIndex < componentSupertypes.length; resultIndex++) {
				result[resultIndex] = TypeFactory.createGenericArrayType(componentSupertypes[resultIndex]);
			}
		}
	...
	public static ParameterizedType capture(ParameterizedType type) {
		...
		return TypeFactory.createParameterizedType(clazz, capturedArguments, ownerType);
	}
