...
import fr.java.maths.geometry.space.shapes.SimpleLine3D;
import fr.java.maths.geometry.space.shapes.Segment3D;
...
public class SpaceTests {
	...
	public static void main(String[] args) {
		...
		SimpleLine3D  L   = SimpleLine3D.of(A, B);
	...
	public static double getDistance(SimpleLine3D _line, Point3D _point) {
		return getDistance(_point, _line);
	}
	public static double getDistance(Point3D _point, SimpleLine3D _line) {
	 ...
	 * PROJECTIONS
	 */
	public static Point3D getProjection(Point3D _point, SimpleLine3D _line) {
		Vector3D AP = Vectors.delta(_point, _line.getPoint());
		Vector3D AB = _line.getDirection();
...
//		throw new ArithmeticException("null vector");
	...
	public static SimpleLine3D getIntersection(Plane3D _planeA, Plane3D _planeB) {
		return null;
	}
	 ...
	 * OTHER TESTS
	 */
	public Point2D transformPoint(Point2D _p, DoubleMatrix33 _T) {
		Vector3D up = Vectors.of(_p.getX(), _p.getY(), 1.0f);
		up = _T.times(up);
		return Points.of(up.getX(), up.getY());
	}
	public Vector2D transformVector(Vector2D _v, DoubleMatrix33 _T) {
		Vector3D uv = new DoubleVector3D(_v.getX(), _v.getY(), 0.0f);
		uv = _T.times(uv);
		return Vectors.of(uv.getX(), uv.getY());
	}

	public Point3D transformPoint(Point3D _p, DoubleMatrix44 _T) {
		DoubleVector4D up = new DoubleVector4D(_p.getX(), _p.getY(), _p.getZ(), 1.0f);
		return Points.of(_T.times(up).downgrade());
	}
	public Vector3D transformVector(Vector3D _v, DoubleMatrix44 _T) {
		DoubleVector4D uv = new DoubleVector4D(_v.getX(), _v.getY(), _v.getZ(), 0.0f);
		return _T.times(uv).downgrade();
	}

	public Vector3D transformPoint(Vector3D _p, DoubleMatrix33 _T) {
		return _T.times(_p);
	}
	public DoubleVector4D transformPoint(DoubleVector4D _p, DoubleMatrix44 _T) {
		return _T.times(_p);
	}
