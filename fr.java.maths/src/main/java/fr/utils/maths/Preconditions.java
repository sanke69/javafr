...
import static fr.utils.maths.Strings.lenientFormat;
 ...
 * {@link Verify} offers assertions similar to those in this class for
 ...
 *
 * <p>
 * {@code Preconditions} uses {@link Strings#lenientFormat} to format error
 * message template strings. This only supports the {@code "%s"} specifier, not
 * the full range of {@link java.util.Formatter} specifiers. However, note that
 * if the number of arguments does not match the number of occurrences of
 * {@code "%s"} in the format string, {@code Preconditions} will still behave as
 * expected, and will still include all argument values in the error message;
 * the message will simply not be formatted exactly as intended.
 *
...
public final class Preconditions {
	...
	public static <T> T nonNull(final T object) {
		return nonNull(object, "Null object is not allowed here.");
	}
