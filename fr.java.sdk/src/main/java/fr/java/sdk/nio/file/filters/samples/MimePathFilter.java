...
import fr.java.sdk.nio.file.filters.PathFilter;

public enum MimePathFilter implements PathFilter {
	...
	@Override
	public boolean test(Path _file) {
		switch(this) {
	...
	private boolean test(Path _file, String[] _extensions) {
		if(_file.toFile().isDirectory())
			return false;

		String ext = _file.toFile().getAbsolutePath().substring(_file.toFile().getAbsolutePath().lastIndexOf("."));
