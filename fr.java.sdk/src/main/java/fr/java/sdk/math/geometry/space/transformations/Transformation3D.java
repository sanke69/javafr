...
public class Transformation3D {
	...
	public static Transformation3D of(Matrix44d _matrix) {
		return new Transformation3D(_matrix);
	}

	protected Matrix44d m;

	public Transformation3D() {
		super();
		m = Matrix44d.identity();
	}
	public Transformation3D(Matrix44d _matrix) {
		super();
		m = _matrix;
	}
	...
	public Point3D 				apply(Point3D _pt) {
		return (Point3D) asUniformMatrix().times(_pt.uniform(1.0)).as3D();
	}
	public Point3D[] 			apply(Point3D[] _ptArray) {
		for(int i = 0; i < _ptArray.length; ++i)
			_ptArray[i] = (Point3D) asUniformMatrix().times(_ptArray[i].uniform()).as3D();
		return _ptArray;
	}
	public List<Point3D>		apply(List<Point3D> _ptList) {
		for(int i = 0; i < _ptList.size(); ++i)
			_ptList.set(i, (Point3D) asUniformMatrix().times(_ptList.get(i).uniform()).as3D());
...
//			pt.set((Vector3D) asUniformMatrix().times(pt.uniform()).as3D());
	...
	public Set<Point3D>			apply(Set<Point3D> _ptSet) {
		...
		for(Point3D pt : _ptSet)
			newSet.add((Point3D) asUniformMatrix().times(pt.uniform()).as3D());
		return newSet;
	}
	/*
	...
	*/
	public Point3DBufferX 		apply(Point3DBufferX _ptBuffer) {
		for(int i = 0; i < _ptBuffer.capacity(); ++i)
			_ptBuffer.put(i, (Point3D) asUniformMatrix().times(_ptBuffer.get(i).uniform()).as3D());
		return _ptBuffer;
	}
	...
	public static Point3D 		transform(Matrix44d m, Point3D p) {
		// TODO:: q = new Matrix3d(i, j, k).times(p).plus(o);
	...
	public static Vector3D 		transform(Matrix44d m, Vector3D v) {
		// TODO:: q = new Matrix3d(i, j, k).times(p);
	...
	public static Vector3D 		transform(Matrix44d m, Vector3D v, double w) {
