...
import fr.media.MediaPlayerAdapter;

public abstract class ThreadedMediaPlayer<T> extends MediaPlayerAdapter<T> implements Runnable {
	private Thread playingThread;

	public ThreadedMediaPlayer() {
		this(false, 25d);
	}
	public ThreadedMediaPlayer(boolean _isInterruptable) {
		this(_isInterruptable, 25d);
	}
	public ThreadedMediaPlayer(boolean _isInterruptable, double _fps) {
	...
	public void 		run() {
		...
		while(!isStopped() && !isFaulted() && !Thread.currentThread().isInterrupted()) {
			...
			try {
				System.out.println(media().frameIndex().get());
